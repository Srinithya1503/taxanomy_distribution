...
output_tsv="taxa_696_dataset.tsv"
...
awk -F'\t' 'NR>1 { print $3 }' "$genome_dataset" | while read -r organism_name; do
    ...
    grep -F "$organism_name" "$rankedlineage" | awk -F'\t\\|\t' -v org="$organism_name" '
    BEGIN {
        OFS = "\t"  # Set the output field separator to a tab
    }
    {
        # Remove leading and trailing spaces from all fields and remove trailing "|"
        for (i = 1; i <= NF; i++) {
            gsub(/^ +| +$|\\|$/, "", $i)
        }
